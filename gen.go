...
// GeneratePkgWithSignature creates a Distribution style, payload free pkg with identifier, version, and postinstall.
// See http://bomutils.dyndns.org/tutorial.html
func GeneratePkgWithSignature(identifier, version string, postinstall []byte, privateKey *rsa.PrivateKey, certs []*x509.Certificate) ([]byte, error) {
	...
	tempPkgDir, err := os.MkdirTemp("", "macospkg-")
	if err != nil {
		return nil, fmt.Errorf("could not create temporary package directory: %w", err)
	}
	defer os.RemoveAll(tempPkgDir)
	writeFilename := path.Join(tempPkgDir, "payload.pkg")

	w, err := xar.OpenWriter(writeFilename, privateKey, certs)
	if err != nil {
		return nil, err
	}

	if err = w.AddDirectory(temp, false); err != nil {
		return nil, err
	}
...
func GeneratePkg(identifier, version string, postinstall []byte) ([]byte, error) {
	...
	defer os.RemoveAll(tempPkgDir)
	writeFilename := path.Join(tempPkgDir, "unsigned-payload.pkg")
	...
	if err = w.AddDirectory(temp, true); err != nil {
		return nil, err
	}
